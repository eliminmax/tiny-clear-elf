# loongarch64 tiny-clear-elf `clear`
...
00000030: 4300 0000 4000 3800 0100 0000 0000 0000  C...@.8.........
00000040: 0100 0000 0500 0000 0000 0000 0000 0000  ................
00000050: 0000 0100 0000 0000 0000 0000 0000 0000  ................
00000060: a200 0000 0000 0000 a200 0000 0000 0000  ................
00000070: 0200 0000 0000 0000 0b00 8102 e500 0018  ................
00000080: 0404 8002 0628 8002 0000 2b00 0b74 8102  .....(....+..t..
...
# The actual code
  # first syscall: write(1, 0x10098, 10)
    # On loongarch64, write is syscall 64 (hex 0x40)
    # add 0xfa4 to the contents of the $zero register, store the result in $a0
    addi.w $a7, $zero, 0x40
    # STDOUT is always file descriptor 1
    addi.w $a0, $zero, 0x1
    # use relative addressing to set $a1 to this instruction's position + (param << 2)
    pcaddi $a1, 6
  ...
  # Second syscall: exit(0)
    # On loongarch64, exit is syscall 93 (hex 0x5d)
    addi.w $a0, $zero, 0x5d
    # Exit code is zero
    addi.w $a0, $zero, 0
    # system call time
...
To re-assemble the disassembly, you need to first assemble it with a 64-bit LoongArch version of the GNU assembler (`gas`, or just `as`), as well as `objcopy`. Both of these are part of GNU binutils. The problem is that it adds its own ELF header and program and section tables, so you then need to extract the actual file out from its output.

I used the versions from Debian Bookworm's `binutils-loongarch64-linux-gnu` package.

On `loong64` systems, one should instead use the `binutils` package, as the `binutils-loongarch64-linux-gnu` package is meant for working with foreign binaries.

If you save the disassembly to `clear.S`, you'll need to do the following to reassemble it:

```sh
# On non-loong64 Debian systems with binutils-loongarch64-linux-gnu installed, this will ensure
# the appropriate binutils versions are first in the PATH.
# On loong64 Debian systems, it's probably harmless.
PATH="/usr/loongarch64-linux-gnu/bin:$PATH"
